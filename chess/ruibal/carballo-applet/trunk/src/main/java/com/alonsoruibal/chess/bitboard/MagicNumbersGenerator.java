...
public class MagicNumbersGenerator {
	...
	long findMagics(byte index, byte m, boolean bishop) {
	  ...
	  for (i = 0; i < (1 << n); i++) {
	    block[i] = AttackTableGenerator.generatePieces(i, n, mask);
		//System.out.println("b:\n" + BitboardUtils.toString(block[i]));
	    attack[i] = bishop ? AttackTableGenerator.getBishopShiftAttacks(BitboardUtils.index2Square(index), block[i]) :
	    	AttackTableGenerator.getRookShiftAttacks(BitboardUtils.index2Square(index), block[i]);
