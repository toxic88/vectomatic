...
public class BoardJPanel extends JPanel implements MouseListener, MouseMotionListener
{
    ...
    */
    public void mouseReleased(MouseEvent e)
    {
        ...
        // notifies move
        chess.userMove(63-originComponent.getIndex(), 63-parent.getIndex());
    }
		...
		while (i < fen.length()) {
			...
			if (p != '/') {
				...
				for (int k = 0; k < (number == 0 ? 1 : number); k++) {
					SquareJPanel panel = (SquareJPanel) chessBoard.getComponent(j++);
					try {
	...
	public void highlight(int from, int to) {
		SquareJPanel squareFrom = (SquareJPanel) chessBoard.getComponent(63-from);
		SquareJPanel squareTo = (SquareJPanel) chessBoard.getComponent(63-to);
